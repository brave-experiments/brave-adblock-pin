...
```bash
cd adblock-version-pinning
npm link          # puts `adblock-pin` on your PATH (symlinked to this repo)
adblock-pin --help
```
...
`node src/pin-adblock.ts <command>`. To remove: `npm rm -g adblock-version-pin`.
...
adblock-pin pin --commit d2d0243c32146ca27b95807d4a36d44664e9b714
...
entries show your pin version (default `99.0.0`).
...
`$XDG_CONFIG_HOME/adblock-pin/config.json` (default `~/.config/adblock-pin/`) and
...
| Symptom | Cause / fix |
|---------|-------------|
| `could not find "Local State" in …` | You passed a path that isn't a Brave user-data or profile dir. Use one of the paths above. |
