...
/**
 * brave-adblock-pin: point a local Brave install at an older version of the adblock
...
const configPath = (): string => {
  ...
  return path.join(base, 'brave-adblock-pin', 'config.json')
}
...
const resolveProfileArg = async (args: Args): Promise<string> => {
  ...
  if (saved) {
    ...
    if (resolved) {
      log(`Using saved profile: ${path.basename(resolved)}  (override with --profile, clear with 'brave-adblock-pin forget')`)
      return resolved
    }
  ...
  if (await promptYesNo('Remember this as the default profile? [Y/n]: ', true)) {
...
const cmdPin = async (userDataDir: string, commit: string, dryRun: boolean) => {
  ...
  const targets = getDefaultEntries(readCatalog(userDataDir)).filter(e => {
    ...
    if (!installed) info(`skip "${e.title}": not installed`)
    return installed
  })
  if (targets.length === 0) {
    die(
      'no default list components are installed to pin.\n' +
        '  Brave may not have downloaded them yet, or a recent "unpin" left them empty\n' +
        '  (Brave deletes superseded versions once it loads a pin). Open Brave for ~a\n' +
        '  minute so it re-downloads the components, then retry.'
    )
  }
  ...
  log('Revert with:  brave-adblock-pin unpin --profile <dir>')
}
...
const USAGE = `brave-adblock-pin: point local Brave at an older version of the adblock lists

Usage:
  brave-adblock-pin pin    [--profile <dir>] --commit <hash> [--dry-run]
  brave-adblock-pin unpin  [--profile <dir>] [--dry-run]
  brave-adblock-pin status [--profile <dir>]
  brave-adblock-pin list
  brave-adblock-pin forget
